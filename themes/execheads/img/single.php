...
<div class="hero" style="background-image: url('<?php bloginfo('template_directory'); ?>/img/brief-backg.jpg');">
        ...
        <div class="column small-12 medium-5 large-4">
          ...
          <form method="get" action="/blog/" class="summary bg--light-gray">
            ...
            <select name="auth">
              ...
              <option value="James Leslie">James Leslie</option>
              <option value="Craig Elvin">Craig Elvin</option>
              <option value="Chris Pestell">Chris Pestell</option>
              <option value="Jennifer Gaster">Jennifer Gaster</option>
            </select>
            <select name="subject">
          ...
          <div class="summary bg--light-gray">
			...
			<?php $posts = get_posts(['post_type' => 'events', 'post_status' => 'publish', 'numberposts' => 4, 'orderby' => 'date', 'order' => 'DESC' ]); ?>
	  		<?php foreach ($posts as $post): ?>
	            <p>
	              <a href="<?php the_permalink(); ?>" class="blue-text"><?php the_title(); ?></a><br />
	              <strong class="futura uppercase gray-text"><small><?php echo get_the_date('d / M / Y'); ?></small></strong>
	            </p>
            <?php endforeach; ?>
            <?php wp_reset_postdata(); ?>
          </div>
